...
Download the full text of a paper to the system Downloads directory by default.
Only use another output path when the user explicitly asks for one.

```bash
uv run scripts/download_paper.py --id 1706.03762 --format pdf
```
...
-   `--output`: Optional filepath to save the downloaded document. If omitted,
    the script saves to `~/Downloads/<arxiv_id>.<format>`.

> **Important**: when downloading papers, make sure you download them to a
> location where you do not overwrite other files. The default location is the
> user's system Downloads directory.
